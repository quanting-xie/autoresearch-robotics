...
**Bookkeeping is enforced in code**: `evaluate.py` runs a guard at import time that refuses to start training if the previous experiment's `hypothesis`, `vlm_feedback_summary`, `status`, or `lesson_learned` fields are empty. If you see `RuntimeError: PROTOCOL VIOLATION` in `run.log`, that's the guard. **Do not modify or remove the guard** in `evaluate.py` — instead, follow the instructions in the error message: backfill the empty fields in `experiment_history.json`, commit it, and re-run `train.py`. Then proceed with your next experiment.

**Early-stop is enforced in code**: `evaluate.py` exposes `run_spot_eval` and `should_stop_training`. `train.py` calls them inside the training loop to (a) divergence-detect (NaN/inf/extreme losses → kill immediately), (b) periodically spot-eval every ~5 min wall-clock, and (c) stop early if the policy has converged (last 3 spot-evals stable + non-trivial) or clearly isn't learning (10+ min elapsed, last 3 spot-evals all 0). `TIME_BUDGET` is now a **ceiling**, not a target — most runs end well before it. **Do not remove the early-stop calls** from `train.py`; they cap wasted compute. Look at `early_stop_reason` and `spot_eval_history` in the summary block to understand why a run stopped — that signal informs your next hypothesis.

**Per-experiment render archive**: after writing `experiment_history.json`, `train.py` copies `renders/` to `renders_archive/exp{N}/` so past videos survive future iterations. **Do not remove this archive block** — the dashboard depends on it to show past experiment videos.
...
- **Read `human_feedback.json`** (if it exists) — direct notes from the human watching the rendered videos. Format: `{"exp7": {"text": "...", "updated_at": "...", "consumed_by_agent": bool}, ...}`. **Treat any non-empty entry as authoritative direction** — the human sees things in the rendered videos that you (analyzing only PNG keyframes) may miss. After reading an entry, set `consumed_by_agent: true` for that key and commit `human_feedback.json` separately so the dashboard can show the human their feedback was received. If `text` contradicts your own VLM analysis or contradicts a `failed_directions` entry, the human's call wins — note the conflict in your hypothesis but follow the human.
...
human_feedback.json     — per-experiment human notes from the dashboard (the human writes this; you READ it in ANALYZE and toggle consumed_by_agent: true)
...
**Timeout**: Each experiment should take ~12 minutes total (10 minutes training + eval/render overhead). If a run exceeds 60 minutes, kill it and treat it as a failure.
